...
import SetDataCoupe from "../ItemsCar/coupe/DataCoupe/sendData";
import cssCoupeMobi from '../ItemsCar/topogarphic/topohaphicMobi.module.scss'
import Plus from "../../../plus/Plus";

import { showMenuCar } from "../ItemsCar/sendans/handleSendans/handleEventOnMobi/showMenuCar";

export default function ChoseTradeCarShowMerCoupeMobi() {
    ...
    let filterCar = data.filter((items) => {
        return items.trademark === "Mercedes"
    })


    return (filterCar.length > 0 ?
        <div className={clsx(cssCoupeMobi.form)}>
            <div className={clsx(cssCoupeMobi.form__container_title, 'title-coupe')}>
                <h2 className={clsx(cssCoupeMobi.title)}>Coupé</h2>
            </div>
